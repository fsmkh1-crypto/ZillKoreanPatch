...
def sync_layout_content(korean: str, layout: str) -> str:
    ...
    Sentence-closing punctuation inserted exactly at an existing layout boundary
    belongs to the preceding visible fragment. Keep it before <line-break> rather
    than moving it to the beginning of the following display line.
    """
    ...
    for tag, i1, i2, j1, j2 in sm.get_opcodes():
        ...
        if i1 == i2:
            if i1 < len(positions):
                start = positions[i1]
                if i1 > 0 and TRAILING_PUNCTUATION_RE.fullmatch(replacement):
