...
// H0/B/A/Combined/Stable-minimal experiments against the CURRENT canonical
// Korean corpus and one authenticated retail asset set. It is diagnostic only:
// the resulting plans are never returned to the production builder.
//
// Because the canonical corpus has continued to evolve since PR #14, this is a
// current-corpus replay of the historical policies, not a byte-for-byte rebuild
// of the historical ISOs. The output says so explicitly to prevent the static
// comparison from being confused with historical runtime evidence.
func auditPR14HistoricalPolicies(root, gameDir string, source *corpus.Project, korean *corpus.KoreanProject) error {
	...
	if err != nil {
		return fmt.Errorf("PR14 policy audit scan BOOT.BIN: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("PR14 policy audit scan bindata.dat: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("PR14 policy audit read H0 EBOOT table: %w", err)
	}
	h0Fixed, err := fixeddata.ParseKoreanEBOOT(h0Data)
	if err != nil {
		return fmt.Errorf("PR14 policy audit parse H0 EBOOT table: %w", err)
	}
	fullData, err := os.ReadFile(filepath.Join(root, "docs", "audit", "fixtures", "pr14-eboot-full.toml"))
	if err != nil {
		return fmt.Errorf("PR14 policy audit read expanded EBOOT fixture: %w", err)
	}
	fullFixed, err := fixeddata.ParseKoreanEBOOT(fullData)
	if err != nil {
		return fmt.Errorf("PR14 policy audit parse expanded EBOOT fixture: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("PR14 policy audit H0 plan: %w", err)
	}
	bPlan, err := koreanslots.BuildPlan(fullTexts, installed, reservedKeys)
	if err != nil {
		return fmt.Errorf("PR14 policy audit B plan: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("PR14 policy audit A plan: %w", err)
	}
	combinedPlan, err := koreanslots.BuildPlan(fullTexts, hanInstalled, reservedKeys)
	if err != nil {
		return fmt.Errorf("PR14 policy audit Combined plan: %w", err)
	}
	stablePlan, stableRelocated, err := replayStableMinimalPlan(h0Plan, fullTexts)
	if err != nil {
		// Historical Stable-minimal did build. If the current corpus makes the old
		// policy impossible, report the drift rather than failing today's build.
		fmt.Printf("FORENSIC PR14_POLICY name=Stable-minimal replay_available=false reason=%q current_corpus_replay=true\n", err.Error())
	}
	...
	fmt.Printf("FORENSIC PR14_POLICY_MATRIX current_corpus_replay=true corpus_entries=%d source_records=%d h0_eboot_fields=%d expanded_eboot_fields=%d installed=%d han_installed=%d\n",
		len(korean.Entries), len(source.Items), len(h0Fixed), len(fullFixed), len(installed), len(hanInstalled))
	...
	for _, row := range rows {
		...
		if auditErr != nil {
			return fmt.Errorf("PR14 policy audit %s exact-byte audit: %w", row.name, auditErr)
		}
		ebootDigest, digestErr := koreanEBOOTEncodingDigest(row.fixed, row.plan.Mapping)
		if digestErr != nil {
			return fmt.Errorf("PR14 policy audit %s EBOOT digest: %w", row.name, digestErr)
		}
		fmt.Printf("FORENSIC PR14_POLICY name=%s mapping_sha256=%s changed_vs_h0=%d custom=%d candidates=%d mapped_exact_hit_records=%d eboot_encoding_sha256=%s current_corpus_replay=true\n",
	...
	fmt.Printf("FORENSIC PR14_RELATION h0_b_mapping_equal=%t a_combined_mapping_equal=%t stable_relocated=%d current_corpus_replay=true\n",
...
func koreanEBOOTEncodingDigest(translations fixeddata.KoreanEBOOTTranslations, mapping koreanslots.Mapping) (string, error) {
	...
	for _, offset := range offsets {
		...
		if err != nil {
			return "", fmt.Errorf("field %#x: %w", offset, err)
		}
