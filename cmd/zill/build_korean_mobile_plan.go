...
//
// Custom Korean allocation is deliberately restricted to installed renderer
// keys whose CP932 bytes decode to exactly one CJK Han rune and encode back to
// the identical byte pair. Device testing confirmed that symbol/circled-number
// keys can be intercepted as UI icons even when their PAF raster is replaced.
func buildKoreanAlphaPlanMobile(root, gameDir string, source *corpus.Project, korean *corpus.KoreanProject) (koreanslots.Plan, int, int, error) {
	...
	fmt.Printf("Korean beta safe-slot allocation: policy=single_han_roundtrip candidates=%d custom=%d headroom=%d enough=%t\n",
	...
	for _, r := range []rune{'게', '깃'} {
		...
		if !ok {
			fmt.Printf("Korean beta safe-slot diagnostic: rune=%q unicode=U+%04X mapped=false\n", r, r)
			continue
		}
		...
		fmt.Printf("Korean beta safe-slot diagnostic: rune=%q unicode=U+%04X key=%04X bytes=% X nominal_cp932=%q\n", r, r, uint16(key), encoded, nominal)
	}
