...
The next high-risk milestone remains the first actual Korean sentence rendered through genuinely reusable renderer slots rather than globally replacing common Japanese glyphs.

Before producing that PoC, the slot candidates must be audited beyond message-corpus usage. At minimum, inspect executable/fixed/UI resources for CP932 references and keep the distinction between `candidate` and `safe/reusable` explicit.

After the wider slot audit and the first end-to-end font/message patch implementation are ready, request another Claude adversarial review before calling the slot set safe or shipping the PoC for PPSSPP observation.
