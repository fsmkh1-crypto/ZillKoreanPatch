# A-038 — Explicit valuescan blind-spot regressions

## Numbering correction

This note was originally created as `A-034-valuescan-explicit-blind-spots.md`, which duplicated the already-existing A-034 standalone value-runtime scanner note. The content and evidence are unchanged; only the audit identifier is corrected to preserve a unique chronological ledger.
...
- **OPEN** for whether either pattern is present in retail code.
...
Once the retail EBOOT is available, does direct inspection around candidate or near-candidate code reveal any of these unsupported dispatch/dataflow shapes?
