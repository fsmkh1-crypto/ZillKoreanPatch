...
public final class FreezeCaptureActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        info.setText("1) PPSSPP에서 원격 디버거를 켜고 포트를 34500으로 맞춥니다.\n"
                + "2) 여기서 '기록 시작'을 누릅니다. 연결될 때까지 자동 재시도합니다.\n"
                + "3) PPSSPP로 돌아가 문제 장면까지 평소처럼 플레이합니다.\n"
                + "4) 앱은 0.5초마다 PC/GPR/CPU 상태를 계속 기록합니다.\n"
                + "5) 프리징이 나면 잠시 그대로 두었다가 이 앱으로 돌아와 '최근 기록 복사'를 누릅니다.\n\n"
                + "중요: 예전에 실제 프리징 로그를 잡았던 장기 샘플링 방식입니다. 프리징 전후 로그가 길어도 정상입니다.");
        info.setTextSize(15);
        ...
        Button startButton = new Button(this); startButton.setText("기록 시작"); startButton.setOnClickListener(v -> startTrace());
    ...
    private void startTrace() { int port = readPort(); if (port < 0) return; Intent intent = new Intent(this, FreezeTraceService.class); intent.setAction(FreezeTraceService.ACTION_START); intent.putExtra(FreezeTraceService.EXTRA_PORT, port); startForegroundService(intent); status.setText("기록 시작됨. 이제 PPSSPP로 돌아가세요. 연결될 때까지 자동 재시도합니다."); }
    private void stopTrace() { Intent intent = new Intent(this, FreezeTraceService.class); intent.setAction(FreezeTraceService.ACTION_STOP); startService(intent); status.setText("기록 중지를 요청했습니다. 마지막 로그는 보존됩니다."); }
